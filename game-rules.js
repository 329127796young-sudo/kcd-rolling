...
(function (root, factory) {
  ...
  function publicPlayerProfile(profile) {
    ...
    const collection = profile.collection && typeof profile.collection === 'object' ? profile.collection : {};
    const diceSkinCollection = profile.diceSkinCollection && typeof profile.diceSkinCollection === 'object' ? profile.diceSkinCollection : {};
    return {
      name: String(profile.name || '旅人').slice(0, 18),
  ...
  function createPlayerState(player = {}) {
    return {
      ...
      profile: publicPlayerProfile(player.profile),
      total: 0,
      roundBank: null,
