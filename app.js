...
// 棋盘主题配置：默认旧橡木保留，贵族大厅·胡桃木作为第二套可用棋盘。
const BOARD_THEMES = Object.freeze({
  'tavern-oak': { name: '墙洞酒馆 · 旧橡木', status: '默认棋盘', base: 'assets/images/boards/tavern-oak/base.webp', wear: 'assets/images/boards/tavern-oak/wear.png', accent: '#d2b96a', enabled: true },
  'noble-hall': { name: '贵族大厅 · 胡桃木', status: '可使用', base: 'assets/images/boards/noble-hall/base.webp', wear: 'assets/images/boards/noble-hall/wear.png', accent: '#c7b59b', enabled: true },
...
const state = {
  target: getOpponentTarget('milo'), playerName: DEFAULT_PLAYER_NAME, playerAvatar: null, playerTotal: 0, opponentTotal: 0, roundScore: 0, rollScoreBase: 0, activeRollIndices: [], round: 1, mode: 'solo',
...
function renderRestartModal() {
  ...
  const stake = Math.max(0, Number(state.match?.stake) || 0);
  const tableMultiplier = getTableMultiplier();
  const personalMultiplier = type === 'stake' ? getActorLossMultiplier('player') : DEFAULT_TABLE_MULTIPLIER;
  const loss = type === 'stake' ? stake * personalMultiplier : 0;
  ...
  if (type === 'stake' && active) {
    ...
    els.restartSettlementPreview.innerHTML = `<div><span>基础赌注</span><b>${formatGroschen(stake)}</b></div><div><span>共享牌桌倍率</span><b>x${tableMultiplier}</b></div><div><span>你的承担倍率</span><b>x${personalMultiplier}</b></div><div><span>新局起始倍率</span><b>x1</b></div><div class="is-loss"><span>本次放弃扣除</span><b>−${formatGroschen(loss)}</b></div>`;
