...
// KC2 规则：每次投掷落地后，必须从"这一掷"新出现的骰子里锁定至少一个能计分的组合，
// 才允许继续投掷或收集分数——不能不选就再掷一次（"空选"）。
// !state.hasRolled：本回合还没掷过，第一掷永远放行。
// state.locked.size === 6：六枚骰子已经全部靠计分锁上（热骰），视为已满足。
// 其余情况下，只有当"这一掷"的骰子里已经锁定了能计分的组合（分数 > 0）才放行。
function hasScoringSelectionThisRoll() {
  if (!state.hasRolled) return true;
  if (state.locked.size === 6) return true;
  return scoreSelection(activeRollValues()).score > 0;
}
...
function updateUI() {
  ...
  els.diceState.textContent = state.rolling ? '骰子在桌面上翻滚……' : state.turn === 'ai' ? '对手正在思考下一掷 · 你可以看见他的骰池变化' : state.gameOver ? '牌局已结束 · 点击重新开始' : !state.hasRolled ? '投掷六枚骰子，点击 3D 骰子锁定任意点数' : mustSelectFirst ? '这一掷有计分骰子 · 请先锁定至少一个才能继续投掷或收集' : available.length === 0 ? '六枚骰子均已锁定 · 可以继续投掷（热骰）或收集' : state.locked.size ? `已锁定 ${state.locked.size} 枚骰子 · 剩余骰子可以继续投掷或收集` : '点击 3D 骰子锁定任意点数';
...
function rollDice(impulse = null) {
  ...
  if (!hasScoringSelectionThisRoll()) { safeAudio('playDeny'); showToast('这一掷里还有计分骰子没选 · 请先锁定至少一个再继续投掷'); return; }
...
function bankScore() {
  ...
  if (!hasScoringSelectionThisRoll()) { safeAudio('playDeny'); showToast('这一掷里还有计分骰子没选 · 请先锁定至少一个再收集'); return; }
