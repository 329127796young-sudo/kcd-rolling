...
const RARITY_META = Object.freeze({
  limited: { label: '限定', className: 'rarity-limited' },
...
function mythicSymbolFace(value) {
  ...
  const symbol = value === 6
    ? `<span class="mythic-wheel-glyph" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i></span>`
    : value === 5
      ? `<span class="mythic-flower-glyph" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></span>`
      : `<span class="mythic-symbol-glyph">${glyph}</span>`;
  return `<span class="mythic-symbol-face" data-symbol-value="${value}" aria-label="${value} 点">${symbol}</span>`;
}
...
const DICE_SKINS = Object.freeze([
  ...
  { id: 'pure-silver', name: '纯银骰子', description: '圆润纯银骰体经过手工抛光，黑化十字花纹嵌入六面，细微拉丝与岁月痕迹在烛光下交替显现。', rarity: 'limited', price: 0, paid: false, limitedReward: true, collectionGroup: 'checkin', unlock: '连续签到 7 天解锁', bodyColor: '#d7d9d8', pipColor: '#252728', roughness: .34, metalness: .96, pipMetalness: .72, pipRoughness: .36, textureProfile: 'pure-silver', bumpScale: .018, pipStyle: 'silver-cross-pips' },
...
const DAILY_CHECKIN_STORAGE_KEY = 'wallhole-daily-checkin-v1';
const DAILY_CHECKIN_REWARD = 150;
const DAILY_CHECKIN_GOAL = 7;
const DAILY_CHECKIN_SKIN_ID = 'pure-silver';
const DEFAULT_DAILY_CHECKIN = Object.freeze({ lastClaimDate: null, streak: 0, totalClaims: 0, silverDiceAwarded: false });
...
// 四名 AI 不再只是同一套评分器换参数，而是各自拥有一套决策哲学。
// 这些值只影响策略偏好，不改变骰子概率、合法性或实际计分规则。
const AI_BEHAVIOR_PROFILES = Object.freeze({
  milo: { strategy: 'harvest', continuationScale: .58, riskScale: 1.28, preserveDiceBonus: 8, bankAt: 285, bankAfterRolls: 2, bankRiskCutoff: .28, raiseBias: -.04, acceptBias: -.04, mistakeMode: 'conservative' },
  vlad: { strategy: 'momentum', continuationScale: 1.42, riskScale: .62, preserveDiceBonus: 25, bankAt: 610, bankAfterRolls: 3, bankRiskCutoff: .46, raiseBias: .12, acceptBias: .08, mistakeMode: 'greedy' },
  marta: { strategy: 'manager', continuationScale: .72, riskScale: 1.42, preserveDiceBonus: 12, bankAt: 250, bankAfterRolls: 2, bankRiskCutoff: .24, raiseBias: -.02, acceptBias: -.03, mistakeMode: 'defensive' },
  musa: { strategy: 'analyst', continuationScale: 1.02, riskScale: .9, preserveDiceBonus: 20, bankAt: 430, bankAfterRolls: 2, bankRiskCutoff: .3, raiseBias: .02, acceptBias: .02, mistakeMode: 'model-error' }
});

function aiTuning(opponentId = 'milo') { return AI_PERSONALITY_TUNING[opponentId] || AI_PERSONALITY_TUNING.milo; }
function aiBehavior(opponentId = state.opponentId) { return AI_BEHAVIOR_PROFILES[opponentId] || AI_BEHAVIOR_PROFILES.milo; }
...
function createAiMind(opponentId = 'milo') {
  ...
  return {
    ...
    mode: 'calm',
    strategy: aiBehavior(opponentId).strategy,
...
function updateAiMode(context = {}) {
  const mind = getAiMind();
  const id = state.opponentId;
  const round = Number(context.round ?? state.round) || 1;
  const lead = Number(context.lead ?? (state.opponentTotal - state.playerTotal)) || 0;
  const projected = Number(context.projected ?? state.opponentRoundScore) || 0;
  const behind = lead < -260;
  const ahead = lead > 260;
  let mode = 'calm';

  if (state.suddenDeath || round >= MATCH_ROUNDS) mode = 'endgame';
  if (id === 'vlad') {
    if (mind.frustration > .46 || (behind && round >= MATCH_ROUNDS - 1)) mode = 'tilted';
    else if (mind.confidence > .68 || projected >= 560) mode = 'confident';
    else if (behind) mode = 'pressured';
  } else if (id === 'marta') {
    if (ahead && projected >= 240) mode = 'protecting-lead';
    else if (behind) mode = 'chasing-safely';
  } else if (id === 'musa') {
    if (mind.frustration > .34) mode = 'rechecking';
    else if (behind || mind.pressure > .42) mode = 'counter-reading';
    else if (projected >= 500) mode = 'high-confidence';
  } else if (id === 'milo') {
    if (ahead && projected >= 260) mode = 'protecting-profit';
    else if (behind) mode = 'quiet-pressure';
  }
  mind.mode = mode;
  mind.strategy = aiBehavior(id).strategy;
  return mode;
}
...
const state = {
  ...
  collectionTab: 'cards', collection: null, diceSkinCollection: null, lottery: null, lotteryRevealActive: false, wallet: null, dailyCheckin: null,
...
const els = {
  ...
  dailyCheckinModal: $('#daily-checkin-modal-backdrop'), dailyCheckinClose: $('#daily-checkin-close'), dailyCheckinClaim: $('#daily-checkin-claim'), dailyCheckinDays: $('#daily-checkin-days'), dailyCheckinStreak: $('#daily-checkin-streak'), dailyCheckinMessage: $('#daily-checkin-message'), dailyCheckinPrizeArt: $('#daily-checkin-prize-art'), dailyCheckinPrizeStatus: $('#daily-checkin-prize-status'),
...
function loadDailyCheckin() {
  try {
    const stored = JSON.parse(window.localStorage.getItem(DAILY_CHECKIN_STORAGE_KEY) || 'null');
    const lastClaimDate = dateKeyOrdinal(stored?.lastClaimDate) === null ? null : String(stored.lastClaimDate);
    return {
      lastClaimDate,
      streak: Math.max(0, Math.floor(Number(stored?.streak) || 0)),
      totalClaims: Math.max(0, Math.floor(Number(stored?.totalClaims) || 0)),
      silverDiceAwarded: Boolean(stored?.silverDiceAwarded)
    };
  } catch {
    return { ...DEFAULT_DAILY_CHECKIN };
  }
}

function saveDailyCheckin() {
  try { window.localStorage.setItem(DAILY_CHECKIN_STORAGE_KEY, JSON.stringify(state.dailyCheckin)); } catch { /* localStorage may be unavailable for file:// pages */ }
}

function dailyCheckinDayDifference() {
  const today = dateKeyOrdinal(localDateKey());
  const last = dateKeyOrdinal(state.dailyCheckin?.lastClaimDate);
  return today === null || last === null ? null : today - last;
}

function hasClaimedDailyReward() {
  return dailyCheckinDayDifference() === 0;
}

function effectiveDailyStreak() {
  const difference = dailyCheckinDayDifference();
  return difference === 0 || difference === 1 ? Math.max(0, state.dailyCheckin?.streak || 0) : 0;
}

function ensureDailyCheckinReward() {
  if (!state.dailyCheckin || !state.diceSkinCollection) return;
  const shouldOwnSilver = state.dailyCheckin.silverDiceAwarded || state.dailyCheckin.streak >= DAILY_CHECKIN_GOAL;
  if (!shouldOwnSilver) return;
  if (!state.diceSkinCollection.ownedSkins.includes(DAILY_CHECKIN_SKIN_ID)) {
    state.diceSkinCollection.ownedSkins.push(DAILY_CHECKIN_SKIN_ID);
    saveDiceSkinCollection();
  }
  if (!state.dailyCheckin.silverDiceAwarded) {
    state.dailyCheckin.silverDiceAwarded = true;
    saveDailyCheckin();
  }
}

function renderDailyCheckinButton() {
  if (!els.dailyCheckinButton || !state.dailyCheckin) return;
  const claimed = hasClaimedDailyReward();
  const streak = effectiveDailyStreak();
  els.dailyCheckinButton.classList.toggle('is-claimed', claimed);
  els.dailyCheckinButton.classList.toggle('has-reward', !claimed);
  if (els.dailyCheckinStatus) els.dailyCheckinStatus.textContent = claimed ? `已签到 · 连续 ${streak} 天` : `今日可领 ${DAILY_CHECKIN_REWARD} 格罗申`;
}
...
function renderDailyCheckin() {
  ...
  const claimed = hasClaimedDailyReward();
  const streak = effectiveDailyStreak();
  const silverOwned = state.diceSkinCollection.ownedSkins.includes(DAILY_CHECKIN_SKIN_ID);
  const activeDay = Math.min(DAILY_CHECKIN_GOAL, claimed ? streak : streak + 1);
  if (els.dailyCheckinStreak) els.dailyCheckinStreak.textContent = String(Math.min(streak, DAILY_CHECKIN_GOAL));
  if (els.dailyCheckinPrizeArt) els.dailyCheckinPrizeArt.innerHTML = renderDiceSkinPreview(diceSkinById(DAILY_CHECKIN_SKIN_ID));
  if (els.dailyCheckinPrizeStatus) els.dailyCheckinPrizeStatus.textContent = silverOwned ? '已经加入骰子收藏' : `还需连续签到 ${Math.max(0, DAILY_CHECKIN_GOAL - streak)} 天`;
  if (els.dailyCheckinDays) {
    els.dailyCheckinDays.innerHTML = Array.from({ length: DAILY_CHECKIN_GOAL }, (_, index) => {
      const day = index + 1;
      const completed = day <= streak;
      const current = !claimed && day === activeDay;
      const reward = day === DAILY_CHECKIN_GOAL ? '<b>150</b><small>格罗申 + 纯银骰子</small>' : '<b>150</b><small>格罗申</small>';
      return `<div class="daily-checkin-day ${completed ? 'is-complete' : ''} ${current ? 'is-current' : ''} ${day === DAILY_CHECKIN_GOAL ? 'is-final' : ''}"><span>第 ${day} 天</span>${reward}<i>${completed ? '✓' : day === DAILY_CHECKIN_GOAL ? '限定' : '¤'}</i></div>`;
    }).join('');
  }
  if (els.dailyCheckinMessage) els.dailyCheckinMessage.textContent = claimed ? `今日已领取 · 当前连续 ${streak} 天` : streak ? `再签到 ${Math.max(1, DAILY_CHECKIN_GOAL - streak)} 天可获得纯银骰子` : '今日奖励等待领取';
  if (els.dailyCheckinClaim) {
    els.dailyCheckinClaim.disabled = claimed;
    els.dailyCheckinClaim.textContent = claimed ? '今日已签到' : `领取 ${DAILY_CHECKIN_REWARD} 格罗申`;
  }
  renderDailyCheckinButton();
}

function claimDailyCheckin() {
  if (!state.dailyCheckin || !state.wallet || !state.diceSkinCollection || hasClaimedDailyReward()) return;
  const difference = dailyCheckinDayDifference();
  state.dailyCheckin.streak = difference === 1 ? Math.max(0, state.dailyCheckin.streak) + 1 : 1;
  state.dailyCheckin.lastClaimDate = localDateKey();
  state.dailyCheckin.totalClaims += 1;
  state.wallet.groschen += DAILY_CHECKIN_REWARD;
  state.wallet.lifetimeEarned += DAILY_CHECKIN_REWARD;
  let unlockedSilver = false;
  if (state.dailyCheckin.streak >= DAILY_CHECKIN_GOAL && !state.diceSkinCollection.ownedSkins.includes(DAILY_CHECKIN_SKIN_ID)) {
    state.diceSkinCollection.ownedSkins.push(DAILY_CHECKIN_SKIN_ID);
    state.dailyCheckin.silverDiceAwarded = true;
    unlockedSilver = true;
  }
  ...
  renderDailyCheckin();
  safeAudio('playBank');
  showToast(unlockedSilver ? `签到成功 · +${DAILY_CHECKIN_REWARD} 格罗申 · 解锁限定纯银骰子` : `签到成功 · +${DAILY_CHECKIN_REWARD} 格罗申`);
}
...
function applyOnlineProfile(profile, settlement = null, forceWallet = false) {
  if (!profile || typeof profile !== 'object') return false;
  const next = {
    groschen: Math.max(0, Math.floor(Number(profile.groschen) || 0)),
    lifetimeEarned: Math.max(0, Math.floor(Number(profile.lifetimeEarned) || 0)),
    lifetimeSpent: Math.max(0, Math.floor(Number(profile.lifetimeSpent) || 0)),
    lastSettlement: profile.lastSettlement || null
  };
  state.onlineProfile = profile;
  // 联机与单机共用同一份账户资料。服务器返回的余额、名片、勋章和骰子皮肤
  // 都写回本地缓存，服务器不可用时仍可用最后一次同步的进度继续单机游玩。
  state.wallet = { ...(state.wallet || DEFAULT_WALLET), ...next };
  if (profile.collection && typeof profile.collection === 'object') {
    state.collection = {
      ...(state.collection || DEFAULT_COLLECTION),
      unlockedCards: Array.isArray(profile.collection.unlockedCards) ? profile.collection.unlockedCards.slice() : [],
      unlockedMedals: Array.isArray(profile.collection.unlockedMedals) ? profile.collection.unlockedMedals.slice() : [],
      purchasedCards: Array.isArray(profile.collection.purchasedCards) ? profile.collection.purchasedCards.slice() : [],
      equippedCard: typeof profile.collection.equippedCard === 'string' ? profile.collection.equippedCard : null,
      equippedMedals: Array.isArray(profile.collection.equippedMedals) ? profile.collection.equippedMedals.slice(0, 3) : []
    };
  }
  if (profile.diceSkinCollection && typeof profile.diceSkinCollection === 'object') {
    state.diceSkinCollection = {
      ...(state.diceSkinCollection || DEFAULT_DICE_SKIN_COLLECTION),
      ownedSkins: Array.from(new Set(['default', ...(Array.isArray(profile.diceSkinCollection.ownedSkins) ? profile.diceSkinCollection.ownedSkins : [])])),
  ...
  renderWallet(); renderPlayerNameUI(); renderPlayerCollection(); updateEquipped();
  if (!els.collectionScreen?.classList.contains('hidden')) renderCollection();
  if (settlement && (settlement.matchId || settlement.winnerSeat !== undefined)) state.onlineSettlement = settlement;
  return true;
}

function syncSharedProfile(reason = 'local-change') {
  const client = window.multiplayerClient;
  if (!client?.connected || !client.syncProfile) return false;
  const snapshot = window.WallholeGame?.getProfileSnapshot?.() || {
    name: state.playerName,
    wallet: state.wallet,
    collection: state.collection,
    diceSkinCollection: state.diceSkinCollection
  };
  snapshot.syncReason = reason;
  return client.syncProfile(snapshot);
}

function matchTypeLabel(type = state.match?.type) {
  return type === 'stake' ? `正式赌局 · 赌注 ${formatGroschen(state.match?.stake || 0)}` : `免费练习桌 · 胜利 +${practiceRewardFor()} 格罗申`;
}

function practiceRewardFor(opponentId = state.opponentId) {
  return PRACTICE_REWARDS[opponentId] || PRACTICE_REWARDS.milo;
}
...
function lotteryCatalogItems() {
  return [
    ...
    ...collectionItems('dice-skins')
      .filter((item) => item.itemId !== 'default' && !item.limitedReward)
...
function renderLotteryCollection() {
  ...
  const activeResult = state.lotteryRevealActive ? last : null;
...
function drawLottery() {
  ...
  state.lotteryRevealActive = true;
...
function renderCollection() {
  ...
  const viewMeta = {
    ...
    'dice-skins': { title: '骰子皮肤', description: '通过购买、签到或抽奖解锁骰子外观；皮肤只改变材质，不影响点数概率与计分。' },
  ...
  const renderItem = (item) => {
    ...
    const action = isUnlocked ? (isEquipped ? '已装备' : '装备') : item.limitedReward ? '签到获得' : lotteryOnly ? '抽奖获得' : item.paid ? (state.wallet?.groschen >= item.price ? '购买' : '余额不足') : '未解锁';
  ...
  if (type === 'cards') {
    ...
    const groups = [
      ...
      { key: 'checkin', title: '酒馆签到限定', subtitle: '连续签到七天解锁 · 不出售、不进入抽奖' },
  ...
  els.collectionContent.querySelectorAll('.collection-item').forEach((item) => item.addEventListener('click', () => {
    ...
    if (type === 'dice-skins') {
      ...
      const nextSkin = state.diceSkinCollection.equippedSkin === key ? 'default' : key;
      if (window.multiplayerClient?.connected) {
        window.multiplayerClient.equipProfile(state.collection, { ...state.diceSkinCollection, equippedSkin: nextSkin });
        showToast('正在同步骰子皮肤装备');
        return;
      }
      state.diceSkinCollection.equippedSkin = nextSkin;
    ...
    const nextCollection = {
      ...state.collection,
      equippedCard: type === 'cards' ? (state.collection.equippedCard === file ? null : file) : state.collection.equippedCard,
      equippedMedals: type === 'medals'
        ? (state.collection.equippedMedals.includes(file) ? state.collection.equippedMedals.filter((id) => id !== file) : [...state.collection.equippedMedals.slice(-2), file])
        : state.collection.equippedMedals
    };
    if (window.multiplayerClient?.connected) {
      window.multiplayerClient.equipProfile(nextCollection, state.diceSkinCollection);
      showToast('正在同步名片与勋章装备');
      return;
    }
...
function purchaseNameCard(card) {
  ...
  if (window.multiplayerClient?.connected) {
    window.multiplayerClient.purchaseProfileItem('card', card.file);
    showToast(`正在从共享钱包购买「${card.name}」`);
    return;
  }
...
function purchaseDiceSkin(skin) {
  ...
  if (window.multiplayerClient?.connected) {
    window.multiplayerClient.purchaseProfileItem('dice-skin', skin.id);
    showToast(`正在从共享钱包购买「${skin.name}」`);
    return;
  }
...
function openCollection(tab = 'cards') {
  closeBoardMenu(); closeOpponentModal(); closeDiceBoxModal(); closeDailyCheckinModal(); closeDetailModal();
  const validTabs = ['cards', 'medals', 'dice-skins', 'dice-loadout', 'lottery'];
  state.collectionTab = validTabs.includes(tab) ? tab : 'cards';
  if (state.collectionTab === 'lottery') state.lotteryRevealActive = false;
  renderCollection();
...
function closeCollection() {
  state.lotteryRevealActive = false;
...
const pages = [
  { kicker: '骰子：基本规则', title: '骰子：基本规则', content: `<div class="codex-columns"><div class="codex-illustration"><div class="codex-die">${pipFace(1)}</div><div class="codex-die">${pipFace(5)}</div><div class="codex-die">${pipFace(6)}</div><div class="codex-die">${pipFace(3)}</div><div class="codex-caption">七枚骰子 · 保留得分 · 继续冒险</div></div><div class="codex-copy"><h3>骰子游戏</h3><p>每个大回合从七枚骰子开始。你可以锁定本次投掷中真正计分的骰子，再用剩余骰子继续投掷，或在安全时收集本轮暂存分。</p><h4>热骰连火倍率</h4><p>六枚骰子完成有效计分时触发<strong>普通热骰</strong>，七枚全部完成有效计分时触发<strong>完美热骰</strong>；两者都会让七枚骰子重新加入下一掷。本轮未触发热骰时为 <strong>x1</strong>；第 1 次连火升至 <strong>x2</strong>，随后依次为 <strong>x3、x5、x8、x12</strong>，第 5 次及之后保持最高 <strong>x12</strong>。</p><p>每次投掷落地后，必须从这一掷里锁定至少一组完整计分组合（1、5、三同或顺子），才能继续投掷或收集；若剩余骰子没有任何得分组合，则触发爆骰，本轮暂存分归零并结束本轮。</p></div></div>` },
  { kicker: '骰子：点数组合', title: '骰子：点数组合', content: `<div class="codex-copy"><p>以下为七骰规则下的组合及其点数值。</p><p>本版本不使用三对，也不加入魔鬼骰；只按下表的单点、三同和顺子计分。</p></div><table class="rule-table"><thead><tr><th>组合</th><th>点数</th><th>说明</th></tr></thead><tbody><tr><td>单个 1</td><td>100</td><td>可单独保留</td></tr><tr><td>单个 5</td><td>50</td><td>可单独保留</td></tr><tr><td>三个相同</td><td>1000 / 200–600</td><td>1 点为 1000，其余点数 × 100</td></tr><tr><td>四个相同</td><td>三同的 2 倍</td><td>三个骰子后每增加一个骰子，点数翻倍</td></tr><tr><td>五至七个相同</td><td>继续翻倍</td><td>每多一枚相同骰子，点数再翻倍</td></tr><tr><td>小顺 1–5</td><td>500</td><td>连续五个点数</td></tr><tr><td>小顺 2–6</td><td>750</td><td>连续五个点数</td></tr><tr><td>大顺 1–6</td><td>1500</td><td>连续六个点数，可与额外 1/5 或三同组合</td></tr><tr><td>普通热骰</td><td>—</td><td>七枚中六枚完成有效计分，额外一枚可舍弃并重新投掷七枚</td></tr><tr><td>完美热骰</td><td>—</td><td>七枚全部完成有效计分，重新投掷七枚</td></tr><tr><td>热骰连火倍率</td><td>x1 → x2 → x3 → x5 → x8 → x12</td><td>未连火为 x1，第 5 次及之后封顶 x12</td></tr></tbody></table>` },
  ...
  { kicker: '骰子：获胜条件', title: '骰子：获胜条件', content: `<div class="codex-columns"><div class="codex-copy"><h3>4 回合比累计分</h3><p>一局固定进行4个大回合。每个大回合先由你完成一轮，再由对手完成一轮；双方本轮最终收集分数都会计入累计战绩。</p><p>第4回合结束后，累计总分更高者获胜。若双方总分相同，则进入突然死亡：下一轮分数更高者立即获胜，若仍平分则继续加赛。</p><h4>收集分数</h4><p>点击“收集”把本轮已保留的点数计入累计战绩，并把本轮结果交给对手。累计分数越高，最后结算时的优势越大。</p></div><div class="codex-illustration"><div class="codex-score">累计总分 <span>最终比较</span></div><div class="codex-score opponent-score-demo">回合 <span>01 / 04</span></div></div></div>` },
...
function raiseTableMultiplier(target, actor = 'player') {
  ...
  if (isOnlineMatch()) {
    ...
    if (!sendOnlineAction({ kind: 'multiplier:propose', target: Number(target) })) { safeAudio('playDeny'); showToast('联机服务器未连接'); return false; }
    return true;
  }
...
function shouldOpponentAcceptRaise(target) {
  ...
  const mind = getAiMind();
  const tuning = aiTuning(state.opponentId);
  const behavior = aiBehavior(state.opponentId);
...
function resolveRaiseOffer(accepted) {
  if (isOnlineMatch()) {
    if (state.match?.pendingRaise?.proposer !== 'ai') return;
    if (!sendOnlineAction({ kind: 'multiplier:respond', accepted: Boolean(accepted) })) { safeAudio('playDeny'); showToast('联机服务器未连接'); }
    return;
  }
...
function chooseOpponentMultiplier() {
  ...
  chance += behavior.raiseBias;
  chance += mind.greed * tuning.greedResponse * .18;
...
function renderMultiplierPanel() {
  ...
  if (els.multiplierHint) {
    if (!activeStake) els.multiplierHint.textContent = state.match?.type === 'practice' ? `练习桌固定 x1 · 胜利 +${practiceRewardFor()} 格罗申` : '正式牌局开始后可加码';
...
function toggleDie(index, source = 'ui') {
  ...
  if (isOnlineMatch()) {
    const nextLocked = !state.locked.has(index);
    if (!sendOnlineAction({ kind: 'lock', index, locked: nextLocked })) { safeAudio('playDeny'); showToast('联机服务器未连接'); return false; }
    return true;
  }
...
function takeScoringDice() {
  ...
  if (isOnlineMatch()) {
    ...
    if (!picks.length) { safeAudio('playDeny'); showToast('这一掷没有可保留的得分骰子'); return; }
    const indices = picks.map((pick) => availableIndices[pick]).filter((index) => Number.isInteger(index));
    if (!sendOnlineAction({ kind: 'lock-many', indices })) { safeAudio('playDeny'); showToast('联机服务器未连接'); }
    return;
  }
  const previousRoundScore = state.roundScore;
  const previousValues = activeRollValues();
...
function rollDice(impulse = null) {
  ...
  if (isOnlineMatch()) {
    if (!sendOnlineAction({ kind: 'roll', impulse: impulse || null })) { safeAudio('playDeny'); showToast('联机服务器未连接'); return; }
    state.rolling = true;
    state.farkle = false;
...
function settleCompletedRound() {
  ...
  if (sudden) {
    ...
    addActivity('ai', '<b>4 回合累计分平手</b> · 进入突然死亡，下一轮高分者立即获胜', '加赛');
    showToast('4 回合累计分平手 · 进入突然死亡');
  }
...
function bankScore() {
  ...
  if (isOnlineMatch()) {
    if (!sendOnlineAction({ kind: 'bank' })) { safeAudio('playDeny'); showToast('联机服务器未连接'); }
    return;
  }
...
function pickAiMistakeCandidate(ranked, behavior) {
  const shortlist = ranked.slice(0, Math.min(4, ranked.length));
  if (shortlist.length < 2) return shortlist[0];
  if (behavior.mistakeMode === 'conservative' || behavior.mistakeMode === 'defensive') {
    return [...shortlist].sort((a, b) => a.risk - b.risk || b.candidate.score - a.candidate.score)[0];
  }
  if (behavior.mistakeMode === 'greedy') {
    return [...shortlist].sort((a, b) => b.candidate.score - a.candidate.score || b.risk - a.risk)[0];
  }
  // 穆萨的错误不是随机乱选，而是把“看起来有前景”的次优方案当成正确答案。
  return shortlist[1 + Math.floor(secureRandomFloat() * Math.min(2, shortlist.length - 1))];
}
...
function chooseOpponentScoringIndices(values) {
  ...
  const tuning = aiTuning(state.opponentId);
  const behavior = aiBehavior(state.opponentId);
  const mode = updateAiMode({ lead: state.opponentTotal - state.playerTotal, projected: state.opponentRoundScore });
  const candidates = generateScoringKeeps(values);
  if (!candidates.length) return [];
  ...
  const ranked = candidates.map((candidate) => {
    ...
    const continuation = remainingSlots.length * 54 * profile.continuationWeight * scoreGrowth * (1 + mind.greed * tuning.greedResponse * .2) * behavior.continuationScale;
    const lookaheadBonus = expectedNextScore * (profile.lookaheadWeight || 0);
    const riskCost = believedRisk * Math.max(projected, 100) * (0.72 + profile.riskTolerance + mind.caution * tuning.lossAversion * .3) * behavior.riskScale;
    ...
    const greedBonus = mind.greed * tuning.greedResponse * Math.max(0, remainingSlots.length - 1) * 18;
    const cautionBonus = mind.caution * tuning.lossAversion * (remainingSlots.length <= 1 ? 26 : 0);
    const preserveDiceBonus = remainingSlots.length * behavior.preserveDiceBonus;
    const leadAfterCandidate = state.opponentTotal + projected - state.playerTotal;
    const strategyBonus = behavior.strategy === 'harvest'
      ? (remainingSlots.length <= 1 ? 18 : -remainingSlots.length * 5)
      : behavior.strategy === 'momentum'
        ? remainingSlots.length * 10 + (candidate.label.includes('顺') ? 28 : 0)
        : behavior.strategy === 'manager'
          ? (leadAfterCandidate > 0 ? -remainingSlots.length * 10 : remainingSlots.length * 4)
          : (candidate.label.includes('顺') ? 34 : 0) + (remainingSlots.length >= 3 ? 16 : 0);
    const modeBonus = mode === 'protecting-lead' || mode === 'protecting-profit'
      ? (remainingSlots.length <= 1 ? 32 : -remainingSlots.length * 8)
      : mode === 'tilted' || mode === 'pressured' || mode === 'counter-reading'
        ? remainingSlots.length * 9
        : mode === 'rechecking'
          ? (candidate.label.includes('顺') ? 22 : 0)
          : 0;
    ...
    return { candidate, risk: believedRisk, utility: scaledCandidateScore + continuation + lookaheadBonus + hotBonus + straightBonus + tripleBonus + finalRoundBonus + threatBonus + greedBonus + cautionBonus + preserveDiceBonus + strategyBonus + modeBonus - riskCost + familyMemory + humanNoise };
  }).sort((a, b) => b.utility - a.utility);

  // 失误是“次优选择”而不是作弊：根据情绪从前三个合法方案中选一个。
  const effectiveMistakeRate = clampAi((profile.mistakeRate || 0) + mind.frustration * tuning.tiltGain * .08 + (mind.greed > .72 ? .025 : 0) + (behavior.mistakeMode === 'model-error' && mode === 'rechecking' ? .06 : 0));
  if (ranked.length > 1 && secureRandomFloat() < effectiveMistakeRate) {
    const choice = pickAiMistakeCandidate(ranked, behavior);
    mind.lastMistakeType = behavior.mistakeMode;
    mind.lastDecision = { action: 'keep', mode: `bounded-mistake-${behavior.mistakeMode}`, label: choice.candidate.label, score: choice.candidate.score, risk: choice.risk };
    rememberAiAction('keep', { label: choice.candidate.label, score: choice.candidate.score, risk: choice.risk, mistake: true, mistakeType: behavior.mistakeMode });
    return choice.candidate.indices;
  }
  mind.lastMistakeType = null;
...
function getPersonalityBankDecision(context) {
  const { profile, behavior, mode, projected, scoreGrowth, believedRisk, remainingSlots, turnRolls, consecutiveContinues, hotChaseCount, totalLeadAfterBank } = context;
  const id = state.opponentId;
  if (projected <= 0) return null;
  const scaledTarget = behavior.bankAt * scoreGrowth;
  const lead = Number(totalLeadAfterBank) || 0;
  const expectedNext = profile.lookaheadSamples
    ? estimateExpectedNextScore(state.opponentActiveIndices, profile.lookaheadSamples)
    : estimateExpectedNextScore(state.opponentActiveIndices, 8);
  let decision = null;
  let modeLabel = null;

  if (id === 'milo') {
    // 米洛的“第二掷收手”是行为规则，不只是更高的 bankProbability。
    if (turnRolls >= behavior.bankAfterRolls && (projected >= scaledTarget || believedRisk >= behavior.bankRiskCutoff || lead >= 220)) {
      decision = true; modeLabel = 'milo-harvest';
    } else if (turnRolls === 1 && projected >= scaledTarget * 1.55 && believedRisk >= .2) {
      decision = true; modeLabel = 'milo-large-safe-bank';
    }
  } else if (id === 'vlad') {
    // 瓦茨拉夫只在热骰追逐达到上限、分数足够厚或决胜阶段领先时刹车。
    if (hotChaseCount >= 2 && projected >= scaledTarget * .9 && believedRisk >= .22) {
      decision = true; modeLabel = 'vlad-hot-chase-brake';
    } else if (turnRolls >= behavior.bankAfterRolls && projected >= scaledTarget && believedRisk >= .3) {
      decision = true; modeLabel = 'vlad-profit-lock';
    } else if (mode === 'endgame' && lead > 180 && projected >= scaledTarget * .72) {
      decision = true; modeLabel = 'vlad-endgame-lock';
    }
  } else if (id === 'marta') {
    // 玛蒂娜先问“这笔分能否封锁分差”，而不是只问还能不能继续。
    if (lead >= 280 && projected >= 190 * scoreGrowth && (turnRolls >= 1 || believedRisk >= .18)) {
      decision = true; modeLabel = 'marta-protect-lead';
    } else if (lead >= 0 && projected >= scaledTarget * .9 && believedRisk >= behavior.bankRiskCutoff) {
      decision = true; modeLabel = 'marta-safe-ledger';
    } else if (lead < -320 && projected < scaledTarget * 1.2 && remainingSlots >= 3 && believedRisk < .3 && turnRolls < 3) {
      decision = false; modeLabel = 'marta-safe-comeback';
    }
  } else if (id === 'musa') {
    // 穆萨比较当前收益与下一掷期望值，保留一个可解释的“验证机会”。
    if (mode === 'endgame' && lead > 0 && projected >= scaledTarget * .72) {
      decision = true; modeLabel = 'musa-endgame-proof';
    } else if (projected >= scaledTarget && (expectedNext < projected * .58 || remainingSlots <= 2)) {
      decision = true; modeLabel = 'musa-expectation-lock';
    } else if (projected < 340 * scoreGrowth && expectedNext > Math.max(projected * .62, 170 * scoreGrowth) && believedRisk < .31 && remainingSlots >= 3) {
      decision = false; modeLabel = 'musa-verify-next-roll';
    }
  }

  if (decision === null) return null;
  const mind = getAiMind();
  mind.lastDecision = { action: decision ? 'bank' : 'continue', mode: modeLabel, projected, risk: believedRisk, expectedNext, remainingSlots, turnRolls, consecutiveContinues };
  rememberAiAction('bank-decision', { decision: decision ? 'bank' : 'continue', mode: modeLabel, projected, risk: believedRisk, expectedNext, remainingSlots, turnRolls, consecutiveContinues });
  return decision;
}
...
function shouldOpponentBank() {
  ...
  const personalityDecision = getPersonalityBankDecision({ profile, behavior, mode: updateAiMode({ lead: totalLeadAfterBank, projected }), projected, scoreGrowth, believedRisk, remainingSlots, turnRolls, consecutiveContinues, hotChaseCount, totalLeadAfterBank });
  if (personalityDecision !== null) return personalityDecision;
  ...
  bankProbability += mind.caution * tuning.lossAversion * .16;
   bankProbability -= mind.greed * tuning.greedResponse * .12;
...
function resetGame(announce = true) {
  ...
  if (els.playerModeCopy) els.playerModeCopy.textContent = state.mode === 'online' ? '联机房间' : '本地练习';
  ...
  if (els.room && state.mode !== 'online') els.room.textContent = state.room;
...
function toggleMode() {
  if (state.mode === 'solo') {
    if (window.multiplayerClient?.openLobby) {
      els.modeLabel.textContent = '联机大厅';
      els.modeToggle.querySelector('i').style.background = '#6fa76b';
      window.multiplayerClient.openLobby();
      showToast('正在打开联机大厅 · 连接本地牌桌服务器');
      return;
    }
    state.mode = 'online';
  } else {
  ...
  const online = state.mode === 'online'; els.modeLabel.textContent = online ? '联机房间' : '本地练习'; els.modeToggle.querySelector('i').style.background = online ? '#6fa76b' : '#83a969';
  if (els.playerModeCopy) els.playerModeCopy.textContent = online ? '联机房间' : '本地练习';
  if (!online) els.room.textContent = state.room;
  showToast(online ? '已进入联机房间' : '已返回本地练习');
}

function isOnlineMatch() {
  return state.mode === 'online' && Boolean(state.match?.online);
}

function sendOnlineAction(action) {
  if (!isOnlineMatch() || !window.multiplayerClient?.sendGameAction) return false;
...
function applyOnlineSnapshot(snapshot) {
  ...
  state.opponentTotal = Number(remote.total) || 0;
  state.playerRoundBank = Number(local.roundBank) || 0;
  state.opponentRoundBank = Number(remote.roundBank) || 0;
  state.round = Number(snapshot.round) || 1;
  state.suddenDeath = Boolean(snapshot.suddenDeath);
  state.turn = Number(snapshot.turnSeat) === Number(state.onlineSeat) ? 'player' : 'ai';
  state.roundScore = Number(local.turnScore) || 0;
  state.rollScoreBase = 0;
  state.dice = Array.isArray(local.dice) ? local.dice.slice() : Array(DICE_COUNT).fill(0);
  state.locked = new Set(Array.isArray(local.locked) ? local.locked : []);
  state.activeRollIndices = Array.isArray(local.activeIndices) ? local.activeIndices.slice() : [];
  state.hasRolled = Boolean(local.hasRolled);
  state.hotDiceCount = Number(local.hotDiceCount) || 0;
  state.hotDiceType = null;
  state.farkle = Boolean(local.farkle);
  state.opponentDice = Array.isArray(remote.dice) ? remote.dice.slice() : [];
  state.opponentActiveIndices = Array.isArray(remote.activeIndices) ? remote.activeIndices.slice() : [];
  state.opponentKeptIndices = Array.isArray(remote.locked) ? remote.locked.slice() : [];
  state.opponentKept = state.opponentKeptIndices.map((index) => state.opponentDice[index]).filter(Boolean);
  state.opponentRoundScore = Number(remote.turnScore) || 0;
  state.opponentHotDiceCount = Number(remote.hotDiceCount) || 0;
  state.opponentHotDiceType = null;
  state.opponentPhase = state.turn === 'ai' ? 'preparing' : 'idle';
  state.gameOver = snapshot.phase === 'finished';
  if (state.match) {
    state.match.active = snapshot.phase !== 'finished';
    state.match.online = true;
    state.match.multiplier = Number(snapshot.multiplier) || 1;
    state.match.scoreMultiplier = Number(snapshot.multiplier) || 1;
    state.match.hasSuccessfulBank = Boolean(snapshot.hasSuccessfulBank);
    state.match.lastRaiseRound = Number(snapshot.lastRaiseRound) || 0;
    state.match.lastRaisedBy = Number(snapshot.lastRaiseBy) === Number(state.onlineSeat) ? 'player' : Number(snapshot.lastRaiseBy) >= 0 ? 'ai' : null;
    const pending = snapshot.pendingRaise;
    state.match.pendingRaise = pending ? { ...pending, proposer: Number(pending.proposer) === Number(state.onlineSeat) ? 'player' : 'ai' } : null;
  }
  renderDice();
  renderOpponentDice();
  updateUI();
...
function animateOnlineRoll(detail) {
  const event = detail.event || {};
  const snapshot = detail.state;
  const ownerIsPlayer = Number(detail.seat) === Number(state.onlineSeat);
  const owner = ownerIsPlayer ? 'player' : 'opponent';
  const values = Array.isArray(event.values) ? event.values : [];
  const indices = Array.isArray(event.indices) ? event.indices : [];
  applyOnlineSnapshot(snapshot);
  if (ownerIsPlayer) { state.rolling = true; state.hasRolled = true; }
  else { state.opponentRolling = true; state.opponentPhase = 'rolling'; }
  els.ritual?.classList.add('active');
  flashBoardLight('roll');
  safeAudio('playShake');
  updateUI();
  ...
  const finish = () => {
    if (settled) return;
    settled = true;
    if (ownerIsPlayer) state.rolling = false; else state.opponentRolling = false;
    els.ritual?.classList.remove('active');
    applyOnlineSnapshot(snapshot);
    safeAudio('playLand');
    flashBoardLight('land');
  ...
  try {
    const started = dicePhysics3D.rollDice(values.length, values, () => { window.clearTimeout(fallback); finish(); }, { owner, indices });
    if (!Array.isArray(started) || started.length !== indices.length) window.setTimeout(finish, 0);
  } catch (error) {
    console.warn('[online:dice:physics]', error);
    window.clearTimeout(fallback);
    window.setTimeout(finish, 0);
  }
}

function handleOnlineGameEvent(detail) {
  if (!isOnlineMatch() || !detail?.state) return;
  const event = detail.event || {};
  if (event.type === 'roll' || event.type === 'farkle') {
    animateOnlineRoll(detail);
    return;
  }
  const ownerIsPlayer = Number(detail.seat) === Number(state.onlineSeat);
  if (event.type === 'lock') {
    const previous = ownerIsPlayer ? new Set(state.locked) : new Set(state.opponentKeptIndices);
  ...
  if (event.type === 'bank') {
    ...
    finishOnlineMatch(detail.state.winnerSeat, detail.settlement || null);
...
function finishOnlineMatch(winnerSeat, settlement = null) {
  if (state.gameOver && state.match?.result) {
    if (settlement) state.onlineSettlement = settlement;
    return;
  }
  const winnerIsPlayer = Number(winnerSeat) === Number(state.onlineSeat);
  state.onlineSettlement = settlement || state.onlineSettlement || null;
  state.gameOver = true;
  state.turn = 'player';
  state.rolling = false;
  state.opponentRolling = false;
  ...
  const name = winnerIsPlayer ? '你' : opponents.remote.name;
...
function enterOnlineMatch(detail) {
  ...
  const seat = Number.isInteger(Number(detail?.seat)) ? Number(detail.seat) : Number(window.multiplayerClient?.seat);
  ...
  state.opponentId = 'remote';
  resetAiMind('remote');
  closeOpponentModal(); closeDiceBoxModal(); closeBoardMenu(); closeCollection(); closeDailyCheckinModal(); closeDetailModal(); closeRestartModal();
  resetGame(false);
  state.match = { type: room.matchType === 'stake' ? 'stake' : 'practice', stake: 0, entryPaid: false, settled: false, active: true, online: true, result: null, payout: 0, multiplier: 1, scoreMultiplier: 1, maxRaiseBy: { player: 1, ai: 1 }, lastRaisedBy: null, lastRaiseRound: 0, raiseCount: 0, hasSuccessfulBank: false, pendingRaise: null };
  applyOnlineProfile(state.onlineProfile, null, true);
  applyOnlineSnapshot(detail.state || room.matchState);
  window.multiplayerClient?.closeLobby?.();
  els.home.classList.add('hidden');
  els.arena.classList.remove('hidden');
  window.requestAnimationFrame(() => dicePhysics3D.resize?.());
  safeAudio('startMusic');
  showToast(`联机牌桌 ${room.roomId || state.room} 已开始 · 服务器裁定骰子结果`);
}

document.addEventListener('wht:online-match-start', (event) => {
  const detail = event.detail || {};
  state.room = detail.room?.roomId || detail.roomId || state.room;
  if (els.modeLabel) els.modeLabel.textContent = '联机房间';
  if (els.modeToggle) {
    els.modeToggle.querySelector('i').style.background = '#6fa76b';
    els.modeToggle.setAttribute('aria-pressed', 'true');
  }
  if (els.playerModeCopy) els.playerModeCopy.textContent = '联机房间';
  if (els.room) els.room.textContent = state.room;
  enterOnlineMatch(detail);
});

document.addEventListener('wht:online-profile', (event) => {
  const detail = event.detail || {};
  const settlement = detail.settlement || null;
  if (!applyOnlineProfile(detail.profile, settlement)) return;
  if (settlement?.reason === 'purchase') {
    const item = settlement.itemType === 'card'
      ? collectionItems('cards').find((entry) => entry.file === settlement.itemId)
      : collectionItems('dice-skins').find((entry) => entry.itemId === settlement.itemId);
    showToast(`已购买「${item?.name || settlement.itemId}」 · ${formatGroschen(settlement.price || Math.abs(settlement.amount || 0))}`);
    safeAudio('playClick');
  } else if (settlement && isOnlineMatch()) {
    const amount = Number(settlement.amount) || 0;
    const message = amount > 0 ? `服务器钱包已更新 · +${amount} 格罗申` : '服务器已记录本局结算';
    showToast(message);
    addActivity('you', `<b>账房</b> · ${message}`, '联机结算');
  }
});
...
document.addEventListener('wht:online-error', (event) => {
  const detail = event.detail || {};
  if (['STORE_ITEM_INVALID', 'STORE_ITEM_OWNED', 'INSUFFICIENT_FUNDS'].includes(detail.code)) {
    showToast(detail.message || '共享商店操作失败');
    return;
  }
  ...
  els.ritual?.classList.remove('active');
  updateUI();
  if (detail.code === 'INVALID_ACTION') safeAudio('playDeny');
});
document.addEventListener('wht:online-match-end', (event) => {
  const detail = event.detail || {};
  if (detail.state) applyOnlineSnapshot(detail.state);
  finishOnlineMatch(detail.winnerSeat ?? detail.state?.winnerSeat, detail.settlement || null);
});
...
function renderOpponentModal() {
  ...
  els.opponentModalCards.innerHTML = Object.entries(opponents).map(([id, profile]) => {
    ...
    const priceCopy = type === 'stake'
      ? `赌注 ${formatGroschen(stake)} · 4 回合对决`
      : `胜利奖励 +${formatGroschen(practiceRewardFor(id))} · 4 回合对决`;
  ...
  if (els.opponentModalStatus) els.opponentModalStatus.textContent = type === 'stake' ? (enough ? `入场需准备最高风险 ${formatGroschen(maxRisk)} · 基础赌注 ${formatGroschen(stake)} · 最高倍率 x${LOSS_MULTIPLIER_CAP}` : `余额不足：至少需准备 ${formatGroschen(maxRisk)}，当前仅有 ${formatGroschen(balance)}`) : `不收取赌注 · 倍率固定 x1 · 赢下一局奖励 ${formatGroschen(practiceRewardFor(selectedId))}`;
...
function openOpponentModal({ preserveType = false } = {}) {
  closeBoardMenu(); closeCollection(); closeDiceBoxModal(); closeDailyCheckinModal(); closeDetailModal();
...
function closeOpponentModal() {
  ...
  if (els.diceBoxModal?.classList.contains('hidden') && els.dailyCheckinModal?.classList.contains('hidden')) document.body.classList.remove('hub-modal-open');
}
...
function openDiceBoxModal() {
  closeBoardMenu(); closeCollection(); closeOpponentModal(); closeDailyCheckinModal(); closeDetailModal(); renderDiceBox();
...
function closeDiceBoxModal() {
  ...
  if (els.opponentModal?.classList.contains('hidden') && els.dailyCheckinModal?.classList.contains('hidden')) document.body.classList.remove('hub-modal-open');
}

function openDailyCheckinModal() {
  closeBoardMenu(); closeCollection(); closeOpponentModal(); closeDiceBoxModal(); closeDetailModal();
  renderDailyCheckin();
  els.dailyCheckinModal?.classList.remove('hidden');
  document.body.classList.add('hub-modal-open');
}

function closeDailyCheckinModal() {
  els.dailyCheckinModal?.classList.add('hidden');
...
function renderDetailModal(kind = 'opponent') {
  ...
  els.detailModalContent.innerHTML = `<div class="detail-profile-line"><div class="detail-avatar opponent-avatar"><img src="${profile.avatar}" alt="${profile.name}头像" /></div><div><b>${profile.role}</b><small>${profile.short} · 4 回合总分制</small></div><span class="role-chip">${profile.riskLabel}</span></div><div class="detail-section"><div class="detail-section-title">打法性格</div><div class="detail-trait-row">${profile.traits.map((trait) => `<span>${trait}</span>`).join('')}</div><p class="detail-note">${profile.style}。AI 会根据当前分差、爆骰风险和剩余骰子动态调整决定。</p></div><div class="detail-section"><div class="detail-section-title">对手骰子组合</div><div class="detail-dice-grid">${profile.loadout.map((id, index) => { const die = dieById(id); const max = Math.max(...die.weights); const favored = die.weights.map((weight, value) => weight === max ? value + 1 : null).filter(Boolean).join('、'); return `<div class="detail-die-row"><span>${index + 1}</span><b>${die.name}</b><small>${die.desc} · 偏好 ${favored || '均衡'}</small></div>`; }).join('')}</div></div>`;
}
...
function confirmRestart() {
  if (isOnlineMatch()) {
    closeRestartModal();
    showToast('联机对局不能本地重开，请离开牌桌后重新创建房间');
    return;
  }
...
function showHome() {
  if (!els.arena?.classList.contains('hidden') && state.match?.active && !state.gameOver) {
    if (isOnlineMatch()) {
      sendOnlineAction({ kind: 'forfeit' });
      state.match.active = false;
      state.gameOver = true;
      window.setTimeout(() => window.multiplayerClient?.leaveRoom?.(), 150);
    } else finishGame('opponent');
  }
  if (state.opponentId === 'remote' || state.mode === 'online') {
    state.mode = 'solo';
    state.opponentId = 'milo';
  ...
  window.clearTimeout(turnTimer); clearOpponentActionTimer(true); closeBoardMenu(); closeCollection(); closeOpponentModal(); closeDiceBoxModal(); closeDailyCheckinModal(); closeDetailModal(); closeRestartModal(); dicePhysics3D.resetOwner?.('opponent'); els.arena.classList.add('hidden'); els.home.classList.remove('hidden'); renderDailyCheckinButton();
}
function startGame() {
  if (!beginMatch()) return;
  safeAudio('startMusic'); closeBoardMenu(); closeCollection(); closeOpponentModal(); closeDiceBoxModal(); closeDailyCheckinModal(); resetGame(false); els.home.classList.add('hidden'); els.arena.classList.remove('hidden'); window.requestAnimationFrame(() => dicePhysics3D.resize?.()); showToast(`${matchTypeLabel()} · 与 ${opponents[state.opponentId].name} 开始对局`);
}

els.roll.addEventListener('click', rollDice); els.bank.addEventListener('click', bankScore); els.restoreLoadout?.addEventListener('click', restoreDefaultLoadout); $('#reset-button')?.addEventListener('click', openRestartModal); els.restart?.addEventListener('click', openRestartModal); els.homeButton.addEventListener('click', showHome); $('#forfeit-button').addEventListener('click', () => { if (state.gameOver) return; if (isOnlineMatch()) sendOnlineAction({ kind: 'forfeit' }); else finishGame('opponent'); });
...
window.addEventListener('keydown', (event) => { if (event.code === 'Space' && !event.repeat) { event.preventDefault(); rollDice(); } if (event.code === 'KeyF' && !event.repeat) { takeScoringDice(); rollDice(); } if (event.code === 'KeyE' && !event.repeat) takeScoringDice(); if (event.code === 'KeyQ' && !event.repeat) bankScore(); if (event.code === 'KeyT' && !event.repeat) openCodex(0); if (event.code === 'KeyR' && !event.repeat) { event.preventDefault(); openRestartModal(); } if (event.code === 'Escape') { closeCodex(); closeBoardMenu(); closeCollection(); closeOpponentModal(); closeDiceBoxModal(); closeDailyCheckinModal(); closeDetailModal(); closeRestartModal(); } });
...
els.dailyCheckinButton?.addEventListener('click', openDailyCheckinModal); els.dailyCheckinClose?.addEventListener('click', closeDailyCheckinModal); els.dailyCheckinClaim?.addEventListener('click', claimDailyCheckin); els.dailyCheckinModal?.addEventListener('click', (event) => { if (event.target === els.dailyCheckinModal) closeDailyCheckinModal(); });
...
// The network client is loaded before app.js. Expose only the small bridge it
// needs after the local profile/loadout state has been initialized.
window.WallholeGame = {
  getLoadout: () => state.loadout.slice(),
...
state.dailyCheckin = loadDailyCheckin();
ensureDailyCheckinReward();
renderPlayerNameUI();
renderWallet();
renderDailyCheckinButton();
...
// 服务器可用时自动同步同一份账户资料，让单机商店与联机大厅共用钱包和收藏；
// 未启动服务器时保持原有 localStorage 单机体验，不阻塞页面加载。
window.setTimeout(() => window.multiplayerClient?.connect?.(), 0);
