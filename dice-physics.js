...
(function createDicePhysics3D(global) {
  ...
  function createPureSilverTexture() {
    return createTextureSet(512, 1.02, function (contexts, size) {
      var random = seededRandom(0x517e2);
      var base = contexts.map.createLinearGradient(0, 0, size, size);
      base.addColorStop(0, '#f1f3f2');
      base.addColorStop(.28, '#c9cdce');
      base.addColorStop(.65, '#9ca3a6');
      base.addColorStop(1, '#5f676a');
      contexts.map.fillStyle = base;
      contexts.map.fillRect(0, 0, size, size);
      fillTexture(contexts.bumpMap, '#808080', size);
      fillTexture(contexts.roughnessMap, '#a6a6a6', size);
      fillTexture(contexts.metalnessMap, '#f4f4f4', size);

      // Fine, uneven hand-polishing lines break the silver highlight into
      // narrow bands without turning the face into a printed stripe pattern.
      for (var line = 0; line < 180; line += 1) {
        var y = random() * size;
        var drift = (random() - .5) * 12;
        contexts.map.beginPath();
        contexts.map.moveTo(-8, y);
        contexts.map.bezierCurveTo(size * .34, y + drift, size * .7, y - drift * .45, size + 8, y + drift * .2);
        contexts.map.strokeStyle = line % 5 === 0 ? 'rgba(255,255,255,.16)' : 'rgba(48,55,58,.075)';
        contexts.map.lineWidth = line % 7 === 0 ? 1.2 : .55;
        contexts.map.stroke();
        contexts.bumpMap.beginPath();
        contexts.bumpMap.moveTo(-8, y);
        contexts.bumpMap.bezierCurveTo(size * .34, y + drift, size * .7, y - drift * .45, size + 8, y + drift * .2);
        contexts.bumpMap.strokeStyle = line % 5 === 0 ? '#8d8d8d' : '#777777';
        contexts.bumpMap.lineWidth = .7;
        contexts.bumpMap.stroke();
      }

      // Sparse oxidation freckles and polishing nicks keep the metal from
      // reading as chrome-plated plastic while preserving its premium sheen.
      for (var mark = 0; mark < 48; mark += 1) {
        var x = random() * size;
        var markY = random() * size;
        var length = 3 + random() * 16;
        contexts.map.beginPath();
        contexts.map.moveTo(x, markY);
        contexts.map.lineTo(x + length, markY + (random() - .5) * 3);
        contexts.map.strokeStyle = mark % 4 === 0 ? 'rgba(28,33,35,.2)' : 'rgba(255,255,255,.12)';
        contexts.map.lineWidth = .5 + random() * .7;
        contexts.map.stroke();
        contexts.roughnessMap.beginPath();
        contexts.roughnessMap.moveTo(x, markY);
        contexts.roughnessMap.lineTo(x + length, markY + (random() - .5) * 3);
        contexts.roughnessMap.strokeStyle = mark % 4 === 0 ? '#d0d0d0' : '#777777';
        contexts.roughnessMap.lineWidth = 1.1;
        contexts.roughnessMap.stroke();
      }
    });
  }
  ...
  function getSkinTextureSet(skin) {
    ...
    if (skin.textureProfile === 'pure-silver') textureSet = createPureSilverTexture();
  ...
  function mythicPetalShape(centerX, centerY, radius, width, rotation) {
    var shape = new THREE_REF.Shape();
    var ux = Math.cos(rotation || 0);
    var uy = Math.sin(rotation || 0);
    var vx = -uy;
    var vy = ux;
    var point = function (along, across) {
      return [centerX + ux * along + vx * across, centerY + uy * along + vy * across];
    };
    var baseLeft = point(-radius * .18, -width * .48);
    var shoulderLeft = point(radius * .34, -width);
    var tip = point(radius, 0);
    var shoulderRight = point(radius * .34, width);
    var baseRight = point(-radius * .18, width * .48);
    shape.moveTo(baseLeft[0], baseLeft[1]);
    shape.lineTo(shoulderLeft[0], shoulderLeft[1]);
    shape.lineTo(tip[0], tip[1]);
    shape.lineTo(shoulderRight[0], shoulderRight[1]);
    shape.lineTo(baseRight[0], baseRight[1]);
    shape.lineTo(baseLeft[0], baseLeft[1]);
    shape.closePath();
    return shape;
  }

  function mythicSpokeShape(rotation, innerRadius, outerRadius, width) {
    var shape = new THREE_REF.Shape();
    var ux = Math.cos(rotation || 0);
    var uy = Math.sin(rotation || 0);
    var vx = -uy;
    var vy = ux;
    var point = function (along, across) {
      return [ux * along + vx * across, uy * along + vy * across];
    };
    var innerLeft = point(innerRadius, -width * .45);
    var outerLeft = point(outerRadius, -width * .7);
    var outerRight = point(outerRadius, width * .7);
    var innerRight = point(innerRadius, width * .45);
    shape.moveTo(innerLeft[0], innerLeft[1]);
    shape.lineTo(outerLeft[0], outerLeft[1]);
    shape.lineTo(outerRight[0], outerRight[1]);
    shape.lineTo(innerRight[0], innerRight[1]);
    shape.closePath();
    return shape;
  }

  function mythicLobedSealShape(lobes, outerRadius, innerRadius, rotation) {
    var shape = new THREE_REF.Shape();
    var steps = 60;
    for (var index = 0; index <= steps; index += 1) {
      var angle = (index / steps) * Math.PI * 2 + (rotation || 0);
      var lobe = (Math.cos(lobes * angle) + 1) * .5;
      var radius = innerRadius + (outerRadius - innerRadius) * Math.pow(lobe, .72);
      var x = Math.cos(angle) * radius;
      var y = Math.sin(angle) * radius;
      if (index === 0) shape.moveTo(x, y); else shape.lineTo(x, y);
    }
  ...
  function createMythicPipGeometry(value) {
    ...
    if (value === 1) {
      ...
      shapes = [mythicRingShape(-.034, 0, .031, .015), mythicRingShape(.034, 0, .031, .015)];
    } else if (value === 3) {
      // A small three-leaf scroll mark.
      shapes = [
        mythicCircleShape(0, .034, .027),
        mythicCircleShape(-.034, -.024, .027),
        mythicCircleShape(.034, -.024, .027)
      ];
    } else if (value === 4) {
      // Four-petal tavern rose: a central diamond with four separated leaves.
      shapes = [mythicPolygonShape(4, .022, 0, Math.PI / 4)];
      for (var petal4 = 0; petal4 < 4; petal4 += 1) {
        var angle4 = petal4 * Math.PI / 2;
        shapes.push(mythicPetalShape(Math.cos(angle4) * .028, Math.sin(angle4) * .028, .035, .011, angle4));
      }
    } else if (value === 5) {
      // Five-petal seal flower: rounded lobes replace the sharp five-point star.
      shapes = [mythicLobedSealShape(5, .066, .028, -Math.PI / 2), mythicRingShape(0, 0, .019, .009)];
    } else {
      // Six-rayed kingdom wheel: a larger open center ring keeps the emblem
      // from collapsing into a solid star when the die is viewed at distance.
      shapes = [mythicRingShape(0, 0, .064, .038), mythicRingShape(0, 0, .024, .011)];
      for (var spoke6 = 0; spoke6 < 6; spoke6 += 1) {
        shapes.push(mythicSpokeShape(spoke6 * Math.PI / 3, .021, .061, .009));
      }
    }
    var geometry;
  ...
  function createSilverCrossPipGeometry(value) {
    if (!THREE_REF || !THREE_REF.ShapeGeometry) return null;
    var key = value === 1 ? 'seal' : 'pip';
    if (silverCrossPipGeometries[key]) return silverCrossPipGeometries[key];
    var radius = value === 1 ? .077 : .052;
    var neck = radius * .29;
    var shoulder = radius * .48;
    var shape = new THREE_REF.Shape();
    var points = [
      [-shoulder, -radius], [shoulder, -radius], [neck, -neck],
      [radius, -shoulder], [radius, shoulder], [neck, neck],
      [shoulder, radius], [-shoulder, radius], [-neck, neck],
      [-radius, shoulder], [-radius, -shoulder], [-neck, -neck]
    ];
    points.forEach(function (point, index) {
      if (index === 0) shape.moveTo(point[0], point[1]); else shape.lineTo(point[0], point[1]);
    });
    shape.closePath();
    var geometry = new THREE_REF.ShapeGeometry(shape, 3);
    geometry.rotateZ(Math.PI / 4);
    geometry.computeVertexNormals();
    silverCrossPipGeometries[key] = geometry;
    return geometry;
  }
  ...
  function applySkin(skin, owner) {
    ...
    ensureRecords(targetOwner).forEach(function (record) {
      ...
      record.mesh.traverse(function (child) {
        if (child.userData && child.userData.isDiePip && child.material) pipMeshes.push(child);
      });
      pipMeshes.forEach(function (child) {
        var useMythicSymbols = skin.pipStyle === 'mythic-symbols';
        var useSilverCrossPips = skin.pipStyle === 'silver-cross-pips';
        var targetGeometry = useMythicSymbols
          ? createMythicPipGeometry(child.userData.diePipValue)
          : useSilverCrossPips
            ? createSilverCrossPipGeometry(child.userData.diePipValue)
        ...
        // Mythic symbols get a slightly oversized, recessed dark copper undercut.
        // It creates a readable shadow edge around the raised emblem without
        // turning the symbol into a glowing UI icon.
        var underlay = child.userData.mythicUnderlay;
        if (useMythicSymbols) {
          if (!underlay) {
            underlay = new THREE_REF.Mesh();
            underlay.userData.dieRecord = record;
            underlay.userData.isMythicPipUnderlay = true;
            underlay.castShadow = false;
            underlay.receiveShadow = false;
            child.parent.add(underlay);
            child.userData.mythicUnderlay = underlay;
          }
          underlay.geometry = targetGeometry;
          if (!underlay.material || !underlay.material.isMeshStandardMaterial) {
            underlay.material = new THREE_REF.MeshStandardMaterial({
              color: '#321017',
              roughness: .48,
              metalness: .5,
          ...
          underlay.material.color.set('#321017');
          underlay.material.roughness = .48;
          underlay.material.metalness = .5;
          underlay.material.needsUpdate = true;
          underlay.position.copy(child.position);
          underlay.quaternion.copy(child.quaternion);
          underlay.scale.set(1.16, 1.16, 1.16);
          var normal = new THREE_REF.Vector3(0, 0, 1).applyQuaternion(child.quaternion).normalize();
          underlay.position.addScaledVector(normal, -.003);
          underlay.renderOrder = (child.renderOrder || 0) - 1;
