...
(function createDicePhysics3D(global) {
  ...
  var SETTLE_CORRECTION_DURATION = .11;
  var LANDING_PREP_HEIGHT = 1.55;
  ...
  function finalizeRecord(record) {
    var quaternion = record.landingQuaternion || (record.settleCorrection ? record.settleCorrection.to : record.mesh.quaternion.clone());
    if (record.body) {
      record.body.quaternion.set(quaternion.x, quaternion.y, quaternion.z, quaternion.w);
  ...
  function updateDice(delta) {
    ...
    rollingDice.slice().forEach(function (record) {
      ...
      var descendingToTable = record.body.position.y < LANDING_PREP_HEIGHT && record.body.velocity.y < 0;
